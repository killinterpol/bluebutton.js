...
Parsers.CCDA.document = function (ccda) {
  ...
  el = doc.tag('id');
  var npi = el.attr('value');

  el = author.tag('assignedPerson').tag('name');
  var name_dict = parseName(el);

  el = author.tag('assignedAuthoringDevice').tag('softwareName');
  var softwareName = el.val();
    
  el = author.tag('assignedAuthoringDevice').tag('manufacturerModelName');
