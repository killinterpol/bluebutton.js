...
var Documents = (function() {
  ...
  var parseDate = function(str) {
    if (!str || typeof str !== 'string') {
      ...
      // Remove - and : from date value but not trailing - that indicates timezone
      if (str.indexOf(":") > 0) {
        var lastColon = str.lastIndexOf(":");
        str = str.substring(0, lastColon + 1).replace("-", "").replace(":", "") +
          str.substring(lastColon + 1);
      }
    }
      ...
      for (var i in telcoEls) {
        if (i < telcoEls.length) {
            ...
            if (tmp.type === "tel" || tmp.type === "mobile") {
                phone.push(obj);
            } else if (tmp.type === "mailto" || tmp.use === "DE" ||
              tmp.use === "SE" || tmp.use === "email") {
                email.push(obj);
            }
        }
      }
